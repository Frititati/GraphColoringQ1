...
void reading_function(int thread_index) {
	string line;
	...
	// REMEMBER!!!!! 3 MAY CHANGE
	int position = number_nodes/3*(thread_index-1)+2;
	...
	if (thread_index == 3)
	 	more =number_nodes - (position-2 + number_nodes/3);

	// cout << "Sono il thread " << thread_index << " e leggo a partire da linea " << position << " fino a " << more << endl;

	for(int i=0; i<number_nodes/3+more; i++) {
		...
		// cout << "Sono il thread " << thread_index << " " << i+number_nodes/3*(thread_index-1)+2 << endl;
		readingMutex.unlock();
